...
//getting the office IATA code
  ...
  if (EXEGYCheck == "TRUE"){
  if (thisOffice != "90212220"){
    send "Please continue in CAIEG3645 office!"
    ask "Please continue in CAIEG3645 office!" assign to qz5
  }
  }
  else {
    if (thisOffice == "90212220"){
    send "This isn't Ex-Egy deal"
    ask "Continue??" assign to qz5
    ask "Continue??" assign to qz5
  }
  }
send "rttn"
capture line:1, column:1, length:16 assign to checkRTTN
if (checkRTTN != "NO ELEMENT FOUND"){
    send "There're tickets issued/voided/refunded!"
...
capture line:2, column:1, length:13 assign to checkNOFXB
if (checkNOFXB== "NO VALID FARE"){
    mandatory ask "There's unavailable class(es)!" assign to qz5
}
...
assign "Please check the baggage in: " to baggageCheckerStatment
    ...
    if (baggageCheckerStatment != "Please check the baggage in: "){
      send "" +baggageCheckerStatment + " Segment(s)"
       ask "Please confirm the baggage to continue!" assign to qz5
    }
    ...
    send "FQC" +totalNewPrice +"EGP/" +firstCurr
          ...
          send "df" +totalOldTSTPrice +"-" +curr2deal
                 ...
                 if (Airline1 =="NP"){
                    send "df" + curr2deal +"*0.05;" +diff
                 }
                 if (Airline1 =="NE"){
                    send "df" + curr2deal +"*0.05;" +diff
                 }
                 if (Airline1 =="MS"){
                    send "df" + curr2deal +"*0.035;" +diff
                 }
      }
      else {
        send "df" + totalOldTSTPrice +"-" +totalNewPrice
               ...
               ask "Continue??" assign to qz5
                ...
                if (PassengerCount>"1"){
                        send "RRN/P1-" +PassengerCount
                    }
                    else{
                        send "RRN/P1"
                    }
                    capture line:1, column:3, length:2 assign to checkAvail
                    if (checkAvail==Airline1){
                        mandatory ask "There's unavailable class(es)!" assign to qz5
												send "IG"
                    }
                    else{
...
send "ER"
if (EXEGYCheck == "TRUE"){
  capture line:1, column:1, length:16 assign to checkKK
  if (checkKK =="VERIFY ITINERARY"){
    choose "Would you like to Confirm?" {
      when ("YES"){
        send "ERK"
        send "RF" +agtName +";ER"
        send "ER"
...
capture line:1, column:1, length:2 assign to checkOKPrint


if (checkOKPrint=="OK"){
    send "IG"
    capture line:1, column:11, length:6 assign to newPNR
    send "RT" + pnr
    send "Xi"
    send "RF" +agtName +";ER"
    send "ER"
    ...
    send "RTA"
  capture line:3, column:58, length:1 assign to checkfirstAirlinePNR
  if (checkfirstAirlinePNR == "/"){
    capture line:3, column:59, length:7 assign to AirlinePNR
  }
  else{
    capture line:4, column:59, length:7 assign to AirlinePNR
  }
    if (AirlinePNR == newPNR){
        send "SOTO DEAL: " +egy1deal + " ,NEW PNR: " +newPNR
    }
    else{
      send "SOTO DEAL: " +egy1deal + " ,NEW PNR: " +newPNR +" ,Airline PNR: " +AirlinePNR
}
